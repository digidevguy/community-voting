...
const noop = {
	limit: async () => ({ success: true, reset: 0, remaining: 9999, limit: 9999, pending: Promise.resolve() })
...
function makeLimiter(tokens: number, window: `${number} ${'s' | 'ms' | 'm' | 'h' | 'd'}`): Ratelimit {
	if (dev) return noop;
