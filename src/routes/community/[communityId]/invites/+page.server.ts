...
import { createInviteSchema, updateInviteSchema } from '$lib/server/communities/communites.validation';
...
export const actions: Actions = {
	...
	create: async ({ locals, request, params }) => {
		...
		const validationResult = createInviteSchema.safeParse({
			...
			label: body.get('label')?.toString() || undefined,
			grantedRole: body.get('grantedRole')?.toString() || undefined,
			membershipDurationDays: body.get('membershipDurationDays')?.toString() || undefined
		});
		...
		try {
			const { expiresAt, maxUses, label, grantedRole, membershipDurationDays } = validationResult.data;
			...
			Sentry.logger.info('Invite created', {
				...
				membershipDurationDays: membershipDurationDays ?? null
			});
	...
	update: async ({ locals, request, params }) => {
		...
		const validationResult = updateInviteSchema.safeParse({
			inviteId: body.get('inviteId')?.toString(),
			expiresAt: body.get('expiresAt')?.toString() || undefined,
			maxUses: body.get('maxUses')?.toString() || undefined,
			label: body.get('label')?.toString() || undefined
		});
		...
		try {
			await updateInvite(locals.db, validationResult.data);
			Sentry.logger.info('Invite updated', {
				userId: locals.user.id,
				communityId: params.communityId,
				inviteId,
				ownInvite: isCreator
			...
			return fail(500, { message: 'Failed to update invite!' });
		}
	}
};
